...
app.post('/api/nutrition/calculate', async (req, res) => {
  try {
    ...
    const nutrition = await nutritionCalculatorService.calculateNutritionForRecipe(recipeId);
    ...
    if (nutrition) {
      ...
      res.status(404).json({
        success: false,
        error: 'Unable to calculate nutrition for this recipe'
      });
    }
  } catch (error) {
...
app.get('/api/recipes/:id/nutrition', async (req, res) => {
  try {
    ...
    let nutrition = await nutritionCalculatorService.getNutritionForRecipe(recipeId);
    
    // If no nutrition data exists, calculate it
    if (!nutrition) {
      nutrition = await nutritionCalculatorService.calculateNutritionForRecipe(recipeId);
    }
...
// @ts-ignore - TypeScript Express typing issue workaround
app.post('/api/recipes/:id/nutrition/scale', async (req, res) => {
  try {
    const recipeId = req.params.id;
    const { servingSize } = req.body;
    
    if (!servingSize || servingSize <= 0) {
      return res.status(400).json({ error: 'Valid serving size is required' });
    }

    const nutrition = await nutritionCalculatorService.getNutritionForRecipe(recipeId);
    
    if (nutrition) {
      const scaledNutrition = nutritionCalculatorService.scaleNutrition(nutrition, servingSize);
      res.json({
        success: true,
      ...
      res.status(404).json({
        success: false,
        error: 'Nutrition data not found for this recipe'
      });
    }
  } catch (error) {
